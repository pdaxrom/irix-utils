build_iperf() {
    ...
    if ! test -e iperf.installed; then
        ...
        pushd ${PKG}-${PKG_VERSION}
            ...
            mkdir -p buildx
            cd buildx
            ../configure ${EXTRA_CONF_OPTS} --prefix=$INST_PREFIX --host=${CROSS_PREFIX} --libdir=$LIBDIR_PREFIX CPPFLAGS="-std=gnu99 -I${INST_PREFIX}/include" LDFLAGS="${COMPAT_IRIX_LIB} -L${LIBDIR_PREFIX} -Wl,-rpath,${LIBDIR_PREFIX}:/opt/irix-gcc-o32/lib${LIBDIR_SUFFIX}"

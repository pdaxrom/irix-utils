build_turbo() {
    ...
    if ! test -e turbo.installed; then
        ...
        pushd ${PKG}
            ...
            mkdir -p buildx
            cd buildx
            cmake -DCMAKE_TOOLCHAIN_FILE=${TOPDIR}/misc/tc-o32.cmake .. -DCMAKE_LIBRARY_PATH=${LIBDIR_PREFIX} -DNCURSESW=${LIBDIR_PREFIX}/libncursesw.so -DNCURSESW_INCLUDE=${INST_PREFIX}/include -DCMAKE_CXX_FLAGS=-I${INST_PREFIX}/include -DCMAKE_EXE_LINKER_FLAGS="-L${LIBDIR_PREFIX} -Wl,-rpath,${LIBDIR_PREFIX} -lcompat_irix"
